...
public sealed class LivePlcTrendAdapter
{
    ...
    private IReadOnlyList<PlcTrendRenderSeries> BuildRenderSeries(IReadOnlyList<PlcTagMonitorViewModel> monitorTags)
    {
        return monitorTags
            .Where(tag => tag.IsTrendVisible)
            .Where(tag => _pointsByTag.ContainsKey(tag.TagId))
            .Select(tag => new PlcTrendRenderSeries(
                tag.TagId,
                tag.Name,
                tag.Unit,
                tag.AxisGroup,
